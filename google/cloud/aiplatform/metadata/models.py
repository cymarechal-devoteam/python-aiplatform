...
def save_model(
    ...
    *,
    ...
    try:
        ...
        if any([isinstance(model, xgb.Booster), isinstance(model, xgb.XGBModel)]):
            framework_name = "xgboost"
            framework_version = xgb.__version__

    if framework_name not in FRAMEWORK_SPECS:
        raise ValueError(
            f"Model type {model.__class__.__module__}.{model.__class__.__name__} not supported."
        )

    save_method = globals()[FRAMEWORK_SPECS[framework_name]["save_method"]]
    model_file = FRAMEWORK_SPECS[framework_name]["model_file"]
    ...
    if not uri:
        ...
        uri = f"{staging_bucket}/{timestamp}-{unique_id}-{framework_name}-model"
...
def _save_input_example(
    ...
    if isinstance(input_example, list):
        if all(isinstance(x, list) for x in input_example):
            example = {
                "type": "list",
                "data": input_example[:MAX_INPUT_EXAMPLE_ROW],
            }
    ...
    if isinstance(input_example, dict):
        if all(isinstance(x, list) for x in input_example.values()):
            example = {
                "type": "dict",
                "data": {
                    k: v[:MAX_INPUT_EXAMPLE_ROW] for k, v in input_example.items()
                },
            }
            ...
            example = {
                "type": "dict",
                "data": {
                    k: v[:MAX_INPUT_EXAMPLE_ROW].tolist()
    ...
    if isinstance(input_example, np.ndarray):
        example = {
            "type": "np.ndarray",
            "data": input_example[:MAX_INPUT_EXAMPLE_ROW].tolist(),
        }
    ...
    try:
        ...
        if isinstance(input_example, pd.DataFrame):
            example = {
                "type": "pd.DataFrame",
                "data": input_example.head(MAX_INPUT_EXAMPLE_ROW).to_dict("list"),
            }
    except ImportError:
        pass
...
def load_model(
    ...
    framework_name = model.framework_name.split(".")[0]

    if framework_name not in FRAMEWORK_SPECS:
        raise ValueError(f"Model type {framework_name} not supported.")

    load_method = globals()[FRAMEWORK_SPECS[framework_name]["load_method"]]
    model_file = FRAMEWORK_SPECS[framework_name]["model_file"]
